...
/* ============ 微信配置 ============ */
const WECHAT = {
  appId: process.env.WX_APPID || 'wxfe942bf48a8d712c', // 正式公众号 AppId
...
/** code -> openid：正式环境请求微信接口换取；失败则回退（避免登录死循环） */
async function exchangeCode(code) {
  if (USE_MOCK) return 'openid_' + hash(code);
  try {
    const url =
...
async function handle(req, res) {
  ...
  if (p === '/api/wechat/logout') {
    res.writeHead(200, {
      'Set-Cookie': 'fx_token=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0',
    });
    return res.end();
  }

  // 3) 开发态模拟登录（非微信环境直接用，便于本地调试 / 演示切换账号）
  //    支持传入 uid 模拟指定微信号；不传 uid 时为每个浏览器会话生成独立 openid，保证不同用户数据隔离
  if (p === '/api/wechat/mock-login' && req.method === 'POST') {
    const body = await readBody(req);
    const uid = body && body.uid;
    const openid = uid ? 'openid_' + hash(String(uid)) : 'openid_' + crypto.randomBytes(8).toString('hex');
    const token = issueToken(openid);
    db.tokens[token] = { openid };
    if (!db.homework[openid]) db.homework[openid] = [];
    if (!db.history[openid]) db.history[openid] = [];
